---
title: Link List
---

## Journal articles

- [Constraining the Milky Way Potential with a Six-Dimensional Phase-Space Map of the GD-1 Stellar Stream](https://ui.adsabs.harvard.edu/abs/2010ApJ...712..260K/abstract)
- [Off the beaten path: Gaia reveals GD-1 stars outside of the main stream](https://arxiv.org/abs/1805.00425)

## Jupyter Notebook

- [How to Use Jupyter Notebook in 2020: A Beginner's Tutorial](https://www.dataquest.io/blog/jupyter-notebook-tutorial/)
- [matplotlib magic](https://ipython.readthedocs.io/en/stable/interactive/magics.html#magic-matplotlib)
- [Try Jupyter quick start](https://jupyter.org/try)

## SQL and ADQL

- [ADQL CONTAINS](https://www.ivoa.net/documents/ADQL/20180112/PR-ADQL-2.1-20180112.html#tth_sEc4.2.12)
- [ADQL documentation](https://www.ivoa.net/documents/ADQL/20180112/PR-ADQL-2.1-20180112.html)
- [An Astronomy Data Query Language cookbook to accompany Gaia Data Release 1](https://www.gaia.ac.uk/data/gaia-data-release-1/adql-cookbook)
- [Astroquery documentation](https://astroquery.readthedocs.io/en/latest/)
- [Gaia ADQL example queries](https://gea.esac.esa.int/archive-help/adql/examples/index.html)
- [SQL (Wikipedia )](https://en.wikipedia.org/wiki/SQL)
- [SQL JOIN](https://www.geeksforgeeks.org/sql-join-set-1-inner-left-right-and-full-joins/)
- [SQL operators](https://www.w3schools.com/sql/sql_operators.asp)
- [SQL USING](https://docs.oracle.com/javadb/10.8.3.0/ref/rrefsqljusing.html)

## GAIA

- [Crossmatch with external catalogues](https://gea.esac.esa.int/archive/documentation/GDR2/Catalogue_consolidation/chap_cu9val_cu9val/ssec_cu9xma/)
- [Crossmatches](https://gea.esac.esa.int/archive/documentation/GDR2/Gaia_archive/chap_datamodel/sec_dm_crossmatches/)
- [Gaia ADQL example queries](https://gea.esac.esa.int/archive-help/adql/examples/index.html)
- [GAIA data release 2 - Homepage](https://www.cosmos.esa.int/web/gaia/dr2)
- [GAIA data release documentation - Main tables](https://gea.esac.esa.int/archive/documentation/GDR2/Gaia_archive/chap_datamodel/sec_dm_main_tables/ssec_dm_gaia_source.html)
- [GAIA Frequently Asked Questions (FAQ)](https://www.cosmos.esa.int/web/gaia/archive-tips)
- [Gaia's Hertzsprung-russell diagram](https://sci.esa.int/web/gaia/-/60198-gaia-hertzsprung-russell-diagram)
- [GAIA TAP+ REST service](https://astroquery.readthedocs.io/en/latest/gaia/gaia.html)
- [Information about GAIA Data Release 2](https://www.cosmos.esa.int/web/gaia/data-release-2)
- [Scanning Law](https://www.cosmos.esa.int/web/gaia/scanning-law)

## Python (General)

- [Strings](https://docs.python.org/3/tutorial/introduction.html#strings)
- [Python String format() Method](https://www.w3schools.com/python/ref_string_format.asp)
- [pandas.DataFrame](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html)
- [NumPy indexing](https://numpy.org/doc/stable/reference/arrays.indexing.html)
- [Context Managers](https://book.pythontips.com/en/latest/context_managers.html)
- [Pandas input output tools (text, CSV, HDF5)](https://pandas.pydata.org/pandas-docs/stable/user_guide/io.html)

## Astropy

- [AstroPy](https://www.astropy.org)
- [AstroPy Tables](https://docs.astropy.org/en/stable/table/)
- [Accessing a Table](https://docs.astropy.org/en/stable/table/access_table.html)
- [Units and Quantities](https://docs.astropy.org/en/stable/units/)
- [Gala](https://gala-astro.readthedocs.io/en/latest/)
- [Source code for gala.coordinates.gd1](https://gala-astro.readthedocs.io/en/latest/_modules/gala/coordinates/gd1.html)
- [gala.coordinates.reflex\_correct](https://gala-astro.readthedocs.io/en/latest/api/gala.coordinates.reflex_correct.html)
- [Matching catalogs](https://docs.astropy.org/en/stable/coordinates/matchsep.html#matching-catalogs)

## Matplotlib

- [Adding subplots](https://matplotlib.org/stable/api/figure_api.html#matplotlib.figure.Figure.add_subplot)
- [Add text to axes with matplotlib.pyplot.text](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.text.html)
- [Annotating text with matplotlib](https://matplotlib.org/stable/tutorials/text/annotations.html)
- [Change the appearance of ticks, tick labels, and gridlines](https://matplotlib.org/stable/api/_as_gen/matplotlib.axes.Axes.tick_params.html)
- [Create a subplot at a specific location inside a regular grid](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.subplot2grid.html)
- [Customizing Matplotlib with style sheets and rcParams](https://matplotlib.org/stable/tutorials/introductory/customizing.html)
- [How to avoid overplotting with python](https://python-graph-gallery.com/134-how-to-avoid-overplotting-with-python/)
- [matplotlib](https://matplotlib.org/)
- [matplotlib magic for Jupyter Notebook](https://ipython.readthedocs.io/en/stable/interactive/magics.html#magic-matplotlib)
- [matplotlib.pyplot.axvline](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.axvline.html)
- [matplotlib.pyplot.plot](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.plot.html)
- [Scatter plots with matplotlib.pyplot.scatter](https://matplotlib.org/3.3.0/api/_as_gen/matplotlib.pyplot.scatter.html)
- [Simple scatter plots](https://jakevdp.github.io/PythonDataScienceHandbook/04.02-simple-scatter-plots.html)
- [Sample matplotlibrc file](https://matplotlib.org/stable/tutorials/introductory/customizing.html#matplotlibrc-sample)
- [Style sheets reference](https://matplotlib.org/stable/gallery/style_sheets/style_sheets_reference.html)
- [Tight Layout guide](https://matplotlib.org/stable/tutorials/intermediate/tight_layout_guide.html)
- [Vertical lines with matplotlib.pyplot.vlines](https://matplotlib.org/stable/api/_as_gen/matplotlib.pyplot.vlines.html)
- [Writing mathematical expressions](https://matplotlib.org/stable/tutorials/text/mathtext.html)

## File Formats

- [Comma-separated values (CSV) (Wikipedia)](https://en.wikipedia.org/wiki/Comma-separated_values)
- [Flexible Image Transport System (FITS) (Wikipedia)](https://en.wikipedia.org/wiki/FITS)
- [Hierarchical Data Format (HDF) (Wikipedia)](https://en.wikipedia.org/wiki/Hierarchical_Data_Format)
- [Multi-Extension FITS File Format](https://hst-docs.stsci.edu/hstdhb/3-hst-file-formats/3-2-fits-file-format)
- [Pandas input output tools (text, CSV, HDF5)](https://pandas.pydata.org/pandas-docs/stable/user_guide/io.html)

## Astronomy background information

- [Betelgeuse](https://simbad.u-strasbg.fr/simbad/sim-basic?Ident=Betelgeuse)
- [Celestial coordinate system (Wikipeda)](https://en.wikipedia.org/wiki/Celestial_coordinate_system)
- [Color-Magnitude and Color-Color plots Overview](https://coolwiki.ipac.caltech.edu/index.php/Color-Magnitude_and_Color-Color_plots_Overview)
- [International Celestial Reference System (ICRS)](https://www.iers.org/IERS/EN/Science/ICRS/ICRS.html)
- [International Celestial Reference System (ICRS) (Wikipedia)](https://en.wikipedia.org/wiki/International_Celestial_Reference_System)
- [International Virtual Observatory Alliance (IVOA) Table Access Protocol (TAP)](https://www.ivoa.net/documents/TAP/)
- [Isochrone interpolation](https://waps.cfa.harvard.edu/MIST/interp_isos.html)
- [MESA Isochrones \& Stellar Tracks](https://waps.cfa.harvard.edu/MIST/)
- [Ordinary stars and HR diagrams](https://spiff.rit.edu/classes/ladder/lectures/ordinary_stars/ordinary.html)
- [Photometric system (Wikipedia)](https://en.wikipedia.org/wiki/Photometric_system)
- [Stellar isochrone (Wikipedia)](https://en.wikipedia.org/wiki/Stellar_isochrone)

## Other background information

- [Figure Accessibility Guidelines](https://journals.aas.org/graphics-guide/)
- [Floating point arithmetic (Wikipedia)](https://en.wikipedia.org/wiki/Floating-point_arithmetic)
- [Indexing](https://chartio.com/learn/databases/how-does-indexing-work/)
- [Markdown guide](https://www.markdownguide.org/)
- [Minutes and seconds of Arc (Wikipedia)](https://en.wikipedia.org/wiki/Minute_and_second_of_arc)
- [Reproducible research (Wikipedia)](https://en.wikipedia.org/wiki/Reproducibility#Reproducible_research)
- [Transpose (Wikipedia)](https://en.wikipedia.org/wiki/Transpose)


